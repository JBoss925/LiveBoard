...
Toolbar sliders use custom range-input track and thumb CSS plus a `--slider-progress` style variable so opacity and stroke-width controls render consistently across Chromium-family browsers and Firefox. Text controls stay visible even when no text shape is selected, but they are disabled and muted unless the current selection contains only unlocked text shapes. Text size uses a pixel number input with increase/decrease buttons; the input keeps a raw draft string while typing and only parses/clamps to `4..512` on blur, Enter, or stepper clicks. Text alignment uses a three-button segmented control.
