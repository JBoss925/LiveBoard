...
Shape patches may include optional `rotation`, `groupId`, and `groupIds`. `rotation` stores degrees for rect-like shapes; line rotation is represented by endpoint updates. Text patches may include `fontSize` from `4` to `512` and `textAlign`, where `textAlign` is one of `left`, `center`, or `right`. `groupIds` is the ordered nesting stack for shape groups, with the last id treated as the active/top group. `groupId` is retained as a compatibility field for older flat groups and stores the first group id in the stack. Setting either group field to `null` removes that field during operation application. Position coordinates are finite world coordinates and may be far outside the initial viewport; validation caps coordinates to a large operational range rather than the default `1200x800` view. Shape dimensions are still bounded separately. `canvas_ops.py` applies batch children in order and derives inverse batch operations from the locked authoritative canvas state.
